...
int carregar_binario(const char *nome_arquivo, ListaPerguntas *lista) {
    ...
    if (!f) return 0; /* arquivo inexistente */
    /* Primeiro lê o tamanho */
    size_t n;
    if (fread(&n, sizeof(size_t), 1, f) != 1) { fclose(f); return 0; }
    inicializar_lista(lista);
    if (!garantir_capacidade) {} /* evita warning; garantido em inicializar_lista */
...
/* Função auxiliar: escreve com tamanho */
int salvar_binario(const char *nome_arquivo, const ListaPerguntas *lista) {
    FILE *f = fopen(nome_arquivo, "wb");
    if (!f) {
        fprintf(stderr, "Erro ao abrir para escrita: %s (%s)\n", nome_arquivo, strerror(errno));
        return 0;
    }
    if (fwrite(&lista->tamanho, sizeof(size_t), 1, f) != 1) { fclose(f); return 0; }
    if (lista->tamanho > 0) {
        if (fwrite(lista->dados, sizeof(Pergunta), lista->tamanho, f) != list
            a->tamanho) { fclose(f); return 0; }
    }
    fclose(f);
    return 1;
}
...
/* CSV simples: campos separados por vírgula, sem aspas handling complexo */
int importar_csv(const char *nome_csv, ListaPerguntas *lista) {
    ...
    while (fgets(linha, sizeof(linha), f)) {
        ...
        if (t < 4) continue; /* formato inválido */
        Pergunta q;
        memset(&q, 0, sizeof(q));
        ...
        if (inserir_pergunta(lista, &q)) ++cont;
    }
    fclose(f);
    return cont;
}
